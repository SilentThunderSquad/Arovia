...
import { Avatar, Box, Button, Card, CardContent, Chip, Grid, IconButton, List, ListItem, ListItemAvatar, ListItemText, Typography } from '@mui/material';
import { CloudUpload, Delete, Description, PictureAsPdf, Visibility } from '@mui/icons-material';
...
const PrescriptionVault = ({ userInfo, onUpdate }) => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            // Backend returns { prescription } — refetch profile to update state
            if (onUpdate) {
    ...
    const handleDelete = async (prescriptionId) => {
        ...
        if (result.isConfirmed) {
            try {
                const data = await userService.deletePrescription(prescriptionId);
                // Backend returns updated prescriptions list
                if (onUpdate) {
                    const profile = await userService.getProfile();
                    onUpdate(profile);
                }
                    ...
                    <List>
                        {userInfo.prescriptions.map((doc, idx) => (
                            <ListItem key={doc._id || idx} sx={{ border: '1px solid #e5e7eb', borderRadius: 2, mb: 1, '&:hover': { bgcolor: '#F8F9FA' } }}
                                secondaryAction={
                                    <Box>
                                        <IconButton edge="end" onClick={() => window.open(doc.path, '_blank')} sx={{ color: '#2EC4B6', mr: 1 }}><Visibility /></IconButton>
