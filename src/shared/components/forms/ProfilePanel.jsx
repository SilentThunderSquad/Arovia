...
import { Avatar, Box, Button, Drawer, FormControlLabel, Grid, IconButton, InputAdornment, MenuItem, Switch, TextField, Typography } from '@mui/material';
import { CameraAlt, Close, UploadFile } from '@mui/icons-material';
...
const ProfilePanel = ({ isOpen, onClose, user, onSave }) => {
    const [formData, setFormData] = useState({
        name: '', email: '', phone: '', dob: '', gender: 'Prefer not to say',
        country: 'India', pincode: '', state: '', city: '',
        address1: '', address2: '', bloodDonation: false, prescription: null
    });
    ...
    useEffect(() => {
        if (user) {
            setFormData(prev => ({
                ...
                dob: user.dob || '', gender: user.gender || 'Prefer not to say',
                country: user.address?.country || 'India', state: user.address?.state || '',
                city: user.address?.city || '', pincode: user.address?.pincode || '',
                address1: user.address?.addressLine1 || '', address2: user.address?.addressLine2 || '',
                bloodDonation: user.bloodDonor || false, prescription: null
            }));
            if (user.profilePicture) {
                const avatarSrc = user.profilePicture.startsWith('http') ? user.profilePicture : `${window.location.origin}${user.profilePicture}`;
    ...
    const handleFileChange = (e) => { const file = e.target.files[0]; if (file) setFormData(prev => ({ ...prev, prescription: file })); };
    ...
    const handleSubmit = (e) => {
        ...
        onClose();
        Swal.fire({ icon: 'success', title: 'Profile Updated', text: 'Your profile details have been saved successfully.', timer: 2000, showConfirmButton: false, customClass: { popup: 'rounded-xl' } });
    };

    const SectionTitle = ({ title }) => (
        <Typography variant="h6" sx={{ color: '#0F4C5C', fontWeight: 700, mb: 2 }}>{title}</Typography>
    );

    return (
        <Drawer anchor="right" open={isOpen} onClose={onClose} transitionDuration={500} PaperProps={{ sx: { width: { xs: '100%', md: '45%' }, maxWidth: '1200px', bgcolor: '#f8f9fa', boxShadow: '-10px 0 50px rgba(0, 0, 0, 0.15)', borderTopLeftRadius: { md: 10 }, borderBottomLeftRadius: { md: 10 }, overflowY: 'auto' } }}>
            <Box sx={{ background: 'linear-gradient(135deg, #0F4C5C 0%, #073B4C 100%)', height: { xs: 64, md: 74 }, position: 'relative', borderTopLeftRadius: { md: 10 }, px: { xs: 2.5, md: 4 }, py: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography variant="h5" sx={{ color: 'white', fontWeight: 700, letterSpacing: '-0.5px' }}>Edit Profile</Typography>
                <IconButton onClick={onClose} sx={{ color: 'white', bgcolor: 'rgba(255,255,255,0.1)', '&:hover': { bgcolor: 'rgba(255,255,255,0.2)' } }}><Close /></IconButton>
            </Box>
                ...
                <form onSubmit={handleSubmit}>
                    <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, alignItems: { xs: 'center', sm: 'flex-end' }, mb: 4, mt: -4.5, gap: 3 }}>
                        <Box sx={{ position: 'relative' }}>
                            <Avatar src={previewUrl} sx={{ width: 140, height: 140, border: '6px solid white', bgcolor: '#2d3748', fontSize: '3rem', fontWeight: 'bold', boxShadow: '0 10px 15px -3px rgba(0, 0, 0, 0.1)' }}>
                                {!previewUrl && user?.name?.charAt(0).toUpperCase()}
                            </Avatar>
                            <IconButton component="label" sx={{ position: 'absolute', bottom: 5, right: 5, bgcolor: '#0F4C5C', color: 'white', border: '2px solid white', '&:hover': { bgcolor: '#093a47' }, width: 40, height: 40 }}>
                        ...
                        <Box sx={{ mb: 1, textAlign: { xs: 'center', sm: 'left' } }}>
                            <Typography variant="h4" fontWeight="800" sx={{ color: '#1a202c' }}>{user?.name}</Typography>
                            <Typography variant="body1" sx={{ color: '#0F4C5C', fontWeight: 500 }}>{user?.email}</Typography>
                            <Typography variant="body2" sx={{ color: '#718096', mt: 0.5 }}>Username: <span style={{ fontWeight: 600, color: '#0F4C5C' }}>@{user?.username || user?.name?.toLowerCase().replace(/\s/g, '')}</span></Typography>
                        </Box>
                    </Box>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 2, pb: 4, mt: 4 }}>
                        ...
                        <Button type="submit" variant="contained" sx={{ borderRadius: 10, textTransform: 'none', fontWeight: 700, bgcolor: '#0F4C5C', color: 'white', px: 6, py: 1.2, boxShadow: '0 4px 14px 0 rgba(15, 76, 92, 0.39)', '&:hover': { bgcolor: '#093a47' } }}>Update</Button>
                    </Box>
                </form>
            </Box>
        </Drawer>
